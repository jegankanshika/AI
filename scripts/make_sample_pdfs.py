"""Generate sample PDF documents (paystub, W-2, bank statement) for the UI demo.

Standalone — depends only on `reportlab`, not on `pytest`.

Usage:
    python scripts/make_sample_pdfs.py [--out-dir samples]

Produces:
    samples/paystub.pdf
    samples/w2.pdf
    samples/bank_statement.pdf
"""
...
def draw_bank_statement(path: Path, *,
...
def main() -> None:
    ...
    p.add_argument("--out-dir", default="samples",
                   help="Directory to write the sample PDFs to (default: samples)")
    args = p.parse_args()

    out = Path(args.out_dir)
    out.mkdir(parents=True, exist_ok=True)

    draw_paystub(out / "paystub.pdf")
    draw_w2(out / "w2.pdf")
    draw_bank_statement(out / "bank_statement.pdf")

    for name in ("paystub.pdf", "w2.pdf", "bank_statement.pdf"):
